...
runlog(fullfile('output','regionStats.log'),'optional');
...
style.BoldFont     = {'FontName',   def.BoldFont};
...
for f = 1:nFields
  ...
  hCB(f) = colorbar('SouthOutside', style.BarStyle {:});
...
for s = 1:nSheets
  ...
  if (exportEps)
    exporting.imagename = [exporting.name '-' sprintf('%03i',(sIndex+1))];
    print2eps(fullfile(exporting.file,[exporting.imagename]),hFig);
  end
...
if (exportVideo)
  stepTimer = tic; runlog([TABS 'Exporting '])
  
  try
    close(mVideoWriter);
  end
  
  mVideoWriter = VideoWriter(exporting.file,'Motion JPEG AVI');
