classdef UniformityPlotOverlay < GrasppeAlpha.Core.Component
  ...
  methods
    ...
    function obj = UniformityPlotOverlay()
    ...
    function delete(obj)
      obj.deleteSubPlots();
      obj.deleteLabels();
      obj.delete@GrasppeAlpha.Core.Component();
      obj.PlotObject = [];
      return;
    end
    ...
    function createLabel(obj, index, region, value)
      try
        if ~isa(obj.PlotObject, 'GrasppeAlpha.Graphics.PlotComponent') || ...
            ~isa(obj.PlotObject.ParentAxes, 'GrasppeAlpha.Graphics.PlotAxes')
          return;
        end
      catch err
        return;
      ...
      try
        ...
        if isempty(label) % Create Label
          try
            label = GrasppeAlpha.Graphics.TextObject(obj.PlotObject.ParentAxes, 'Text', int2str(index)); ...
    ...
    function updateLabel(obj, index)
      try
        ...
        try
          % cMap    = colormap(obj.PlotObject.ParentAxes.Handle);
          % cLimit  = get(obj.PlotObject.ParentAxes.Handle, 'clim');
          pValue  = obj.LabelValues(index);
          ...
          [pColor pRev] = obj.PlotObject.ParentAxes.GetMapColor(pValue);
          
          if pRev % max(pColor) < 0.75 && mean(pColor)<0.75 % min(pColor) < 0.5 && (max(pColor) < 0.5 || mean(pColor)<0.5)
            obj.LabelObjects{index}.Color = 'w';
          else
            obj.LabelObjects{index}.Color = 'k';
          end
      ...
      try
        ...
        for m = 1:zoneCount %numel(obj.LabelObjects)
          ...
          if zoneArea(1)>7 && zoneArea(2)>7
            ...
            hold(parentAxes, 'on');
            subPlot1 = fill3( ...
              xData, yZoneData, zData + 1.5, zonePlotFace, parentOpt{:}, ...
            ...
            hold(parentAxes, 'on');
            subPlot3 = fill3( ...
              xData, yMeanData, zData + 1.25, meanPlotFace, parentOpt{:}, ...
