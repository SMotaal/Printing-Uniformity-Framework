function r = testPerformance( obj )
  ...
  caseIDs                   = obj.Reader.Cases.keys;
  caseCount                 = numel(caseIDs);
    
  regionModeIDs             = obj.Reader.regionModeRules(1, 1);
  regionModeCount           = numel(regionModeIDs);  
  regionModeDescription     = obj.Reader.regionModeRules(1, 3);
  
  delayedUpdate             = obj.DelayedUpdate;
  obj.DelayedUpdate         = false;
  ...
  for m = 1:caseCount
    ...
    caseData                = obj.Reader.Cases(caseID);
    ...
    dispf('\t\tChange Case:\t%s (%d of %d)', caseName, m, caseCount);
    
    setIDs                  = sort(caseData.Index.PatchSets);
    setCount                = numel(setIDs); % caseData.Length.PatchSets;
    ...
    for n = 1:setCount
      ...
      dispf('\t\t\tChange Set:\t%s (%d of %d)', setName, n, setCount);
      ...
      for p = 1:regionModeCount
        ...
        dispf('\t\t\t\tChange Sheet:\t%s (%d of %d)', sheetName, q, sheetCount);
  ...
  obj.DelayedUpdate         = delayedUpdate;
