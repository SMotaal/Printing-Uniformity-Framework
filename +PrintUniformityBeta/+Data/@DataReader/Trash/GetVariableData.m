function [ variableData parameters ] = GetVariableData(obj, sheetID) % newData, parameters, setData, )
  ...
  if ~variableReady && exist('sheetID', 'var') 
    if isscalar(sheetID)
      ...
      throw(PrintUniformityBeta.Data.ReaderException.SheetRangeError([], sheetID, dataReader.Parameters));
    end
  end
...
function variableData = getRawData(parameters, setData, sheetID)
  ...
  try
    ...
    throw( ...
      PrintUniformityBeta.Data.ReaderException.SheetRangeError(err, sheetID, parameters));
  end
