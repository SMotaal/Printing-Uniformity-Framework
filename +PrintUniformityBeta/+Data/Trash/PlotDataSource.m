classdef PlotDataSource < PrintUniformityBeta.Data.DataSource
  ...
  methods
    ...
    function obj = PlotDataSource(varargin)
      ...
      plotObject = [];
      try
        if GrasppeAlpha.Graphics.PlotComponent.checkInheritence(varargin{1})
...
%       try obj.Reader.Parameters.SheetID = obj.SheetID; end
  ...
  methods
    ...
    function ProcessCaseData(obj, eventData)
      obj.ProcessCaseData@PrintUniformityBeta.Data.DataSource(eventData);
      obj.ProcessSetData();
    end
    
    function ProcessSetData(obj, eventData)
      if nargin>1
        obj.ProcessSetData@PrintUniformityBeta.Data.DataSource(eventData);
      end
    ...
    function ProcessVariableData(obj, eventData)
      ...
      if nargin>1
        obj.ProcessVariableData@PrintUniformityBeta.Data.DataSource(eventData);
      end
    ...
    function ProcessSheetData(obj, eventData)
      ...
      obj.UpdatePlotData;
      if nargin>1      
        obj.ProcessSheetData@PrintUniformityBeta.Data.DataSource(eventData);
      end
    end
...
%         try UI.setStatus(state, obj.PlotObjects(1).ParentFigure.Handle); end
      ...
      obj.OnDataLoad@PrintUniformityBeta.Data.DataSource(eventData);
    end
...
%           try UI.setStatus('Updating...', obj.PlotObjects(1).ParentFigure.Handle); end
      ...
      obj.OnDataSuccess@PrintUniformityBeta.Data.DataSource(eventData);
      
      % try GrasppeKit.Utilities.DelayedCall(@(s, e) UI.setStatus('', obj.PlotObjects(1).ParentFigure.Handle), 1, 'start'); end
    end
    
    function OnDataFailure(obj, eventData)
      % try UI.setStatus('Terminating...', obj.PlotObjects(1).ParentFigure); end
      obj.OnDataFailure@PrintUniformityBeta.Data.DataSource(eventData);
  ...
  methods
    ...
    function attachPlotObject(obj, plotObject)
      ...
      if isempty(plotObject) || ~GrasppeAlpha.Graphics.PlotComponent.checkInheritence(plotObject)
        return;
      end
  ...
  methods (Access=protected)
    ...
    function ResetPlot(obj, plotObject)
      try
        if GrasppeAlpha.Graphics.PlotComponent.checkInheritence(plotObject)
    ...
    function updatePlots(obj, linkedPlots)
      ...
      try
        ...
        if exist('plotObject', 'var') && ...
            GrasppeAlpha.Graphics.PlotComponent.checkInheritence(plotObject)
