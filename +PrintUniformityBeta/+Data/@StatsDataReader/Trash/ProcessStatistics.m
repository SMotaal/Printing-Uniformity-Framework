function [ dataSource stats ] = ProcessStatistics( dataSource, dataSet, regions  )
  ...
  for r = 1:numel(regionNames)
    ...
    regionID        = DataReader.GenerateCacheID([],dataSet, regionName);
    ...
    %       ~(sa(regionStatsData(1).Stats, 'GrasppeAlpha.Stats.TransientStats'))
    %     regionStatsData = [];
    %   end
    % catch err
    %   regionStatsData = [];
