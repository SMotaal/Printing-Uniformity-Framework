classdef PatchStatsDataSource < Grasppe.PrintUniformity.Data.UniformityDataSource
  ...
  methods (Hidden)
    function obj = PatchStatsDataSource(varargin)
...
%       end
    ...
    function [sheetStats] = getSheetStatistics(obj, sheetID, variableID)
      ...
      sheetStats = struct('Data', [], 'Masks', [], 'Values', [], 'Regions', []);
      ...
      try
        ...
        sheetStats = struct('Data', newData, 'Masks', regionMasks, 'Values', regionStats, 'Regions', regionRects);
...
%           end
        ...
        if isempty(sheetStats), return; end
        
        %newData         = sheetStats.Data;
        obj.PlotRegions = sheetStats.Regions;
        obj.PlotValues  = sheetStats.Values;
        
        %Z     = squeeze(newData);
        %[X Y] = meshgrid(1:size(newData, 3), 1:size(newData, 2));
        
        regionX = sheetStats.Regions(:,[1 3]);
        regionY = sheetStats.Regions(:,[2 4]);
        
        X = regionX(:, [1 2 2 1 1])';
        Y = regionY(:, [1 1 2 2 1])';
        Z = repmat(sheetStats.Values, 1, 5)';
        
        %for r = 1:size(sheetStats.Regions,1)
        %  region  = sheetStats.Regions(r,:);
          
          %px      = region([1 4]);
          %plotbox = patch(px([1 2 2 1 1]), py([1 1 2 2 2]), 'w', 'ZData', [1 1 1 1 1] * pz-1, 'Parent', label.ParentAxes.Handle, 'linewidth', 1, 'LineStyle', '-', 'EdgeColor', 'w', 'EdgeAlpha', 0.25, 'FaceColor', 'none', 'FaceAlpha', 0.25, 'linesmoothing', 'on');
        %end
