classdef MetaProperty < GrasppeAlpha.Core.Prototype
  ...
  methods (Static)
    ...
    function properties = Get(grouping, varargin)
      properties = GrasppeAlpha.Core.MetaProperty.MetaPropertiesRecord(grouping, varargin{:});
    end
    ...
    function propertyMeta = Declare(name, definingClass, displayName, category, mode, description)
      ...
      propertyMeta  = GrasppeAlpha.Core.MetaProperty.Define( ...
    ...
    function propertyMeta = CreateDuplicate(source, varargin)
      ...
      propertyMeta = GrasppeAlpha.Core.MetaProperty;
      ...
      GrasppeAlpha.Core.MetaProperty.MetaPropertiesRecord(propertyMeta.Grouping, propertyMeta);
    end
    ...
    function properties = MetaPropertiesRecord(mediatorID, varargin)
      ...
      try
        ...
        if nargout==0 && nargs>0
          if nargs==1 && isa(varargin{1}, 'GrasppeAlpha.Core.MetaProperty')
